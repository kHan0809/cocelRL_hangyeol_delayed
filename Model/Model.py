...
class State_Predict(nn.Module):
    def __init__(self, state_dim, action_dim, delay_len,device):
        ...
        self.N = nn.Linear(self.state_dim + self.action_dim , self.state_dim)
        self.D = nn.Linear(self.state_dim + self.action_dim , self.state_dim)
        self.F = nn.Linear(self.state_dim + self.action_dim , self.state_dim)


    def forward(self,state,action_buffer):
        # o, hidden = self.GRU(state, self.hidden0)
        for i in range(self.delay):
            state = self.Predict(state,torch.reshape(action_buffer[i],(1,1,-1))) # (to match state dim) #(batch, seqence, input size)
    ...
    def forward_batch(self,batch_size,state,action_buffer):
        ...
        for i in range(self.delay):
            state = self.Predict(state, action_buffer[:,i,:].unsqueeze(1))
            # o, hidden = self.GRU(state, hidden)
        return state
    ...
    def forward_4_train(self,batch_size,state,action_delayed):
        ...
        state = state.unsqueeze(1)
        # o, hidden = self.GRU(state, hidden0)
        state = self.Predict(state, action_delayed)
        return state

    def Predict(self,state,action):
        D = self.D(torch.cat((state, action), 2))
        N = self.N(torch.cat((state, action), 2))
        F = self.F(torch.cat((state, action), 2))
