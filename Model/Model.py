...
class Critic4Delay(nn.Module):
    def __init__(self,state_dim, action_dim, delay):
        super(Critic4Delay, self).__init__()
        self.state_dim  = state_dim
        self.action_dim = action_dim
        self.delay      = delay

        self.fc1 = nn.Linear(self.state_dim    + self.action_dim*(delay+1), 400)
        self.fc2 = nn.Linear(400, 300)
        self.fc3 = nn.Linear(300, 1)

        self.q2_fc1 = nn.Linear(self.state_dim + self.action_dim*(delay+1), 400)
        self.q2_fc2 = nn.Linear(400, 300)
        self.q2_fc3 = nn.Linear(300, 1)
...
class State_Predict(nn.Module):
    def __init__(self, state_dim, action_dim, delay_len,device):
        ...
        self.F = nn.Sequential(
            nn.Linear(self.state_dim + self.action_dim + self.state_dim, self.state_dim),
            nn.Sigmoid()
        )
    ...
    def forward(self,state,action_buffer):
        ...
        for i in range(self.delay):
            state = self.Predict(state,torch.reshape(action_buffer[i],(1,1,-1)),o)
